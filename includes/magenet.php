...
class Magenet
{
    ...
    {
        $this->links_db_file = WP_UNIVERSAL_MAGENET_DIR.'assets/'._MN_USER.'mn-links.db';
        $this->getLinksArray();
    ...
    public function getLinks($show = 0)
    {
        ...
        if ($this->page_links) {
            foreach ($this->page_links as $link) {
                $mads_block .= "\n" . "<div class='mads{$this->links_counter}'>" . $link . "</div>";
    ...
    {
        $links_page_url = urldecode($links_page_url);
        $page_url = urldecode($page_url);

        if ($this->comparePaths($page_url, $links_page_url)) {
            return true;
        }

        if ($links_page_url == $_SERVER['SCRIPT_URI'])
            return true;

        if (strpos($page_url, '?') > -1 && strpos($links_page_url, '?') > -1) {

            $page_url_array = parse_url($page_url);
            $links_page_url_array = parse_url($links_page_url);

            if ($this->comparePaths($page_url_array['path'], $links_page_url_array['path'])) {
    ...
    private function comparePaths($a = null, $b = null)
    {
        if ($a && strlen($a) > 0 && $b && strlen($b) > 0) {
            ...
            if ($a[strlen($a) - 1] == "/" && substr($a, 0, -1) == $b) {
                return true;
            }

            if ($b[strlen($b) - 1] == "/" && substr($b, 0, -1) == $a) {
                return true;
            }
        }
